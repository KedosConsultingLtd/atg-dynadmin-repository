...
// ==/UserScript==
// 0.4 - New Graphing Library - old one wasn't working in Firefox for some reason.
// 0.35 - switched to use the old approach for viewing sub-elements - charts still broken at the moment.
...
itemDescTRs.each(function (i, elem) {
    ...
    itemMiss.push(itemMissCnt > 0 ? [idName,parseFloat(itemMissCnt)] : [idName, 0]);
    
    queryMiss.push(queryMissCnt > 0 ? [idName,parseFloat(queryMissCnt)] : [idName, 0]);
}); 
//[1]).parent().next().next()
...
$('<h1 name="charts">Cache Miss Charts</h1>').appendTo($('body'));

console.log(itemMiss);
